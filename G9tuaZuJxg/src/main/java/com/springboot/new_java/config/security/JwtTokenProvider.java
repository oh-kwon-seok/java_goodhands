...
public class JwtTokenProvider {
    ...
    // @Value를 static 필드에 주입하기 위한 setter
    @Value("${springboot.jwt.secret}")
    public void setSecretKey(String key) {
        JwtTokenProvider.secretKey = key;
    }
    ...
    protected void init(){
        ...
        if (secretKey == null || secretKey.isEmpty()) {
            throw new IllegalStateException("springboot.jwt.secret 설정이 없습니다!");
        }

        // secretKey가 너무 짧으면 패딩 처리
        if (secretKey.length() < 32) {
            LOGGER.warn("[init] secretKey가 너무 짧습니다. 32자로 패딩합니다.");
            secretKey = String.format("%-32s", secretKey).replace(' ', '0');
        }

        System.out.println("Original secretKey: " + secretKey);
