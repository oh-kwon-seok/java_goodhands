    ...
    public String getKakaoAccessToken(String code) {
        ...
        try {
            ...
            ResponseEntity<KakaoTokenResponseDto> response = restTemplate.exchange(
                    requestURL, HttpMethod.POST, request, KakaoTokenResponseDto.class);

            return response.getBody().getAccess_token();

        } catch (Exception e) {
            LOGGER.error("카카오 토큰 발급 실패: {}", e.getMessage(), e);
    ...
    public SignInResultDto kakaoSignIn(String code, String clientIp) throws RuntimeException {
        try {
            ...
            if (existingUser.isPresent()) {
                ...
                LOGGER.info("[kakaoSignIn] 카카오 신규 회원 가입 완료: {}", user.getName());
            }
