...
import com.springboot.new_java.data.repository.seniorDisease.SeniorDiseaseRepository;
...
public class SeniorService extends AbstractCacheableSearchService<Senior, SeniorDto> {
    ...
    public SeniorService(SeniorRepository seniorRepository, UserRepository userRepository, DiseaseRepository diseaseRepository, SeniorDiseaseRepository seniorDiseaseRepository,CareHomeRepository careHomeRepository, RedisTemplate<String, Object> redisTemplate, ObjectMapper objectMapper) {
        super(redisTemplate, objectMapper);
        this.seniorRepository = seniorRepository;
        this.userRepository = userRepository;
        this.careHomeRepository = careHomeRepository;
        this.seniorDiseaseRepository = seniorDiseaseRepository;
    ...
    public Senior performSave(SeniorDto seniorDto) {
        ...
        Long userId = seniorDto.getUser_id();
        Long caregiverId = seniorDto.getCaregiver_id();

        Optional<User> user = userRepository.findByUid(userId);
        Optional<User> caregiver = userRepository.findByUid(caregiverId);
        ...
        senior.setUser(user.get());
        senior.setCaregiver(caregiver.get());
        ...
        Senior insertSenior = seniorRepository.save(senior);
        Long uid = insertSenior.getUid();


        List<Map<String, Object>> seniorDiseaseList = seniorDto.getSenior_disease();

        if(seniorDiseaseList != null){
            for (Map<String, Object> seniorDiseaseData : seniorDiseaseList) {
                SeniorDisease seniorDisease = new SeniorDisease();

                seniorDisease.setSenior(insertSenior);

                if (seniorDiseaseData.containsKey("disease_uid")) {
                    Long diseaseUid = Long.parseLong(seniorDiseaseData.get("disease_uid").toString());

                    Disease selectedDisease = diseaseRepository.findByUid(diseaseUid);
                    seniorDisease.setDisease(selectedDisease);

                }

                seniorDisease.setDiagnosis_date(seniorDiseaseData.get("diagnosis_date").toString());
                seniorDisease.setNote(seniorDiseaseData.get("note").toString());
                seniorDisease.setUse_auto_checklist(Boolean.valueOf(seniorDiseaseData.get("use_auto_checklist").toString()));

                LOGGER.info("시니어디지즈 : {}", seniorDisease);
    ...
    public Senior performUpdate(SeniorDto seniorDto) {
        ...
        Senior updateSenior = seniorRepository.save(senior);
        Long uid = updateSenior.getUid();

        List<Map<String, Object>> seniorDiseaseList = seniorDto.getSenior_disease();

        if(seniorDiseaseList != null){

            List<SeniorDisease> deletedData = seniorDiseaseRepository.findBySeniorUid(seniorDto.getUid());
            LOGGER.info("deletedData : {}",deletedData.size());
            deletedData.forEach(sd ->
                    LOGGER.info("삭제 대상: id={}, senior_uid={}, disease_uid={}",
                            sd.getUid(),
                            sd.getSenior() != null ? sd.getSenior().getUid() : "null",
                            sd.getDisease() != null ? sd.getDisease().getUid() : "null")
            );
            seniorDiseaseRepository.deleteAll(deletedData);
            ...
            for (Map<String, Object> seniorDiseaseData : seniorDiseaseList) {
                ...
                seniorDisease.setSenior(updateSenior);
    ...
    public SeniorDto convertToDto(Senior senior) {
        ...
        if (senior.getUser() != null) {
            dto.setUser_id(senior.getUser().getUid());
        }
        if (senior.getCaregiver() != null) {
            dto.setCaregiver_id(senior.getCaregiver().getUid());
        }
        if (senior.getCareHome() != null) {
            dto.setCare_home_uid(senior.getCareHome().getUid());
        }

        dto.setCaregiver(senior.getCaregiver());
        dto.setUser(senior.getUser());
        dto.setUse_care_schedule(senior.getUse_care_schedule());
        dto.setCare_schedule_protocol(senior.getCare_schedule_protocol());


        List<SeniorDisease> seniorDiseases = seniorDiseaseRepository.findBySeniorUid(senior.getUid());



        List<Map<String, Object>> seniorDiseaseList = Optional.ofNullable(seniorDiseases)
                .orElse(Collections.emptyList())
                .stream()
                .map(sd -> {
                    Map<String, Object> diseaseMap = new HashMap<>();
                    diseaseMap.put("disease_uid", sd.getDisease().getUid());
                    diseaseMap.put("disease_name", sd.getDisease().getName());
                    diseaseMap.put("diagnosis_date", sd.getDiagnosis_date());
                    diseaseMap.put("note", sd.getNote());
                    diseaseMap.put("use_auto_checklist", sd.getUse_auto_checklist());
                    return diseaseMap;
                })
                .collect(Collectors.toList());

        dto.setSenior_disease(seniorDiseaseList);
