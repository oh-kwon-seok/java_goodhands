...
public class KakaoAuthService {
    ...
    public SignInResultDto kakaoSignIn(String code, String clientIp) throws RuntimeException {
        try {
            ...
            // 5. 온보딩 완료된 기존 회원 - 권한 체크
            String userAuth = user.getAuth();
            if (userAuth == null ||
                    (!userAuth.equals("ROLE_CARE") && !userAuth.equals("ROLE_GUARDIAN"))) {

                LOGGER.warn("[kakaoSignIn] 승인되지 않은 사용자 로그인 시도: userId={}, auth={}",
                        user.getId(), userAuth);

                SignInResultDto unauthorizedResult = new SignInResultDto();
                unauthorizedResult.setSuccess(false);
                unauthorizedResult.setCode(-2);
                unauthorizedResult.setMsg("승인이 되지 않았습니다");
